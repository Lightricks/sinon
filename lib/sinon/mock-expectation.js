...
var spyWithArgs = require("./spy").withArgs;
...
var mockExpectation = {
    ...
    create: function create(methodName) {
        ...
        expectation.instantiateFake = mockExpectation.create;
    ...
    verifyCallAllowed: function verifyCallAllowed(thisValue, args) {
        var expectedArguments = this.expectedArgumentsForArgs(args);
    ...
    withArgs: function withArgs() {
        return spyWithArgs.apply(this, arguments);
    },
    ...
    expectedArgumentsForArgs: function expectedArgumentsForArgs(args) {
        var matchingFakes = this.matchingFakes(slice.call(args));

        if (!matchingFakes.length) {
          return null;
        }

        // Getting the most similar fake by taking the one with the most arguments.
        var mostSimilarFake = matchingFakes.reduce(function(mostSimilarSoFar, fake) {
          if (fake.matchingArguments.length > mostSimilarSoFar.matchingArguments.length) {
            return fake;
          }
          return mostSimilarSoFar;
        });

        return mostSimilarFake.matchingArguments;
    },
    ...
    toString: function () {
        var args = (this.matchingArguments || []).slice();
